...
const STEPS = ['Details', 'Content', 'Pricing', 'Delivery', 'Media', 'Preview']
const PATHS = ['details', 'content', 'pricing', 'delivery', 'media', 'preview'].map((s) => `/creator/products/create/${s}`)

export default function ProductBuilderShell({ step, children, onContinue, canContinue = true, continueLabel = 'Continue' }: { step: number; children: ReactNode; onContinue: () => void; canContinue?: boolean; continueLabel?: string }) {
  const navigate = useNavigate()
  const back = () => (step === 0 ? navigate('/creator/products/create') : navigate(PATHS[step - 1]))
  return (
    <div className="flex h-full flex-col bg-bg">
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-border bg-bg/90 px-2 backdrop-blur-md" style={{ paddingTop: 'var(--safe-top)' }}>
        ...
        <h1 className="absolute left-1/2 -translate-x-1/2 text-[15px] font-bold text-ink">Create Product</h1>
      ...
      <div className="shrink-0 border-t border-border bg-bg/95 px-[22px] pt-3 backdrop-blur-md" style={{ paddingBottom: 'calc(14px + var(--safe-bottom))' }}>
        <div className="flex gap-2.5">
          <SecondaryButton onClick={() => navigate('/creator/products')} className="min-w-[104px]">Save Draft</SecondaryButton>
